...
<div class="wrapper col2">
  <div id="topnav">
    <ul>
      ...
      <li><a href="cloudlogin.jsp">Cloud</a>
       
      </li>
      <li><a href="grouplogin.jsp">Group</a></li>
      <li><a href="dataownerlogin.jsp">TPA</a></li>
      
      <li class="last"><a href="proxylogin.jsp">Proxy</a></li>
    </ul>
  </div>
</div>
<!-- ####################################################################################################### -->
...
<div class="wrapper col4">
 ...
 Connection con = ConnectionProvider.getConn();
         ...
         String sql="SELECT * FROM graphtbl2 where user_id='"+user_id+"'and fileid='"+id+"'";
         st=con.createStatement();
         rs=st.executeQuery(sql);
         while(rs.next())
         {       
             arr1.add(rs.getString("fileid"));
             arr2.add(rs.getString("DecryptTime"));
             System.out.println("Fileid"+arr1);
             System.out.println("DecryptTime"+arr2);
             
             arr111.add(rs.getString("fileid"));
             arr222.add(rs.getString("downloadTime"));
             System.out.println("Fileid"+arr111);
             System.out.println("DownloadTime"+arr222);      
            }
        ...
        System.out.println("DecryptTime"+arr22);
        ...
        System.out.println("DownloadTime"+arr2222);
    ...
    <div style="width:30%; height:30px; float:left"><div style="background-color:#FFB6C1; float:left; width:10px; height:10px"> </div><div style="float:left; margin-left:10px">File id--<%= arr11 %></div></div>
		...
		var barChartData = {
			labels : ["Decryption Time","Download Time"],
			datasets : [
				
				{
					
					fillColor : "rgba(248,187,229,1.5)",
		...
		window.onload = function () {
			var chart = new CanvasJS.Chart("chartContainer", {
				...
				data: [{
					type: "column",
					dataPoints: [
						{ y: <%=arr22%>, label: "Decryption Time" },
						{ y: <%=Float.toString(aa)%>, label: "Download Time" },
